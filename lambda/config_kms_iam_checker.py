...
Rule Name:
  key-with-iam-access-checker

Description:
  Check that key resource policies do not allow IAM User Permissions. The rule is NON_COMPLAINT if the policy allows IAM User permissions. 
  This rule does not mark AWS-managed Keys as not compliant.

Trigger:
  Periodic, Configuration Changes, Oversized Configuration Changes
...
Scenarios:
  ...
  Scenario 1:
  Given: Config rule lambda script does not have permission to read key resource policy.
   Then: Return COMPLIANT (or add COMPLIANT entry to evaluations array)

  Scenario 2:
  Given: Config rule lambda script can access key resource policy (based on IAM readonly permission)
    And: KeyManager is NOT AWS
   Then: Return NON_COMPLIANT (or add NON_COMPLIANT entry to evaluations array)

  Scenario 3:
  Given: Config rule lambda script can access key resource policy (based on IAM readonly permission)
    And: KeyManager IS AWS
   Then: Return NOT_APPLICABLE  (or add NOT_APPLICABLE entry to evaluations array)

  Scenario 4:
  Given: key_id is not found (this should never occur)
   Then: Return None
...
# This generate an evaluation for config (for periodic runs)
def build_evaluation(resource_id, compliance_type, event, resource_type=DEFAULT_RESOURCE_TYPE, annotation=None):
    # code from: https://github.com/awslabs/aws-config-rules/blob/master/python/KMS_KEYS_TO_NOT_DELETE/KMS_KEYS_TO_NOT_DELETE.py
...
# function to process each key for complaince
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if periodic==1:
      ...
      # use a paginator in case there are more than 100 keys
      p=kms_client.get_paginator('list_keys')
      ...
      for page in paginator:
        ...
        for key in keys:
          ...
          # call the check_key_policy function for each key to determine compliance (AWS managed keys are "NOT_APPLICABLE")
          compliant=check_key_policy(key_id)

          if DEBUG==1:
            print(key_id,"compliance:",compliant)
            
          if compliant==0:
            evaluations.append(build_evaluation(key_id, 'NON_COMPLIANT', event, 'AWS::KMS::Key'))
          elif compliant==1:
            evaluations.append(build_evaluation(key_id, 'COMPLIANT', event, 'AWS::KMS::Key'))
          else:
            evaluations.append(build_evaluation(key_id, 'NOT_APPLICABLE', event, 'AWS::KMS::Key'))
      ...
      # if not period change, process the 1 key only
      # Resource Change Check
      
      if DEBUG==1:
        print("Change run or manual check run detected")

      # make sure the key_id is found
      try:    
        key_id=json.loads(event['invokingEvent'])['configurationItem']['configuration']['keyId']
        if DEBUG==1:
          print(key_id)
      except Exception as e:
        if DEBUG==1:
          print(e)
          print("Line 209: key_id not found")
      ...
      #return compliant or not (or not applicable if its an aws managed key)
      if DEBUG==1:
        print(key_id,"compliance:",compliant)
